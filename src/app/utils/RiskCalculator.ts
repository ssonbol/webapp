...
class RiskCalculation {
  ...
  strokeRS = (params: strokeRS) => {
    let score: number = 0;
    let percentage: { symbol: string, value: number };
    ...
    if (score < 50) percentage = { symbol: '<', value: 3 };
    else if (score < 75) percentage = { symbol: '<', value: 28 };
    else if (score < 90) percentage = { symbol: '<', value: 75 };
    else if (score >= 90) percentage = { symbol: '>', value: 75 };
